...
<section class="content" id="content">
 <div class="content-title">
     ...
     <?php $ReportId1 = App\VendorPaymentReport\VendorPaymentReport::latest()->take(1)->get(); ?>
 ...
 <table class="items">
   ...
   <tbody>
      @foreach ($reports as $item)
         <tr>
            <td class="ac">
               <?php $getPRNumber = App\VendorPayment\VendorPayment::where('id','=',$item->vendor_payment_id)
            ...
            <td class="ac">
                {{$item->vendor_bills->number}}<br>
             </td>
             <td class="ac">
                {{moneyFormat($item->vendor_bills->total, $PRNumber->currency, true)}}<br>
             </td>
            <td class="ac">
                @if($item->amount_applied == 0)
                --
                @else
                {{moneyFormat($item->amount_applied, $PRNumber->currency, true)}}
                @endif
            </td>
            <td class="ac">
                @if($item->amount_applied_lbp == 0)
                    --
                @else
                    LBP {{moneyFormat($item->amount_applied_lbp, $PRNumber->currency, false)}} / {{$item->amount_applied_lbp_rate}}
                @endif
            </td>
            {{-- <td class="ac">
                @if($item->amount_applied_vat == 0)
                    --
                @else
                 LBP {{moneyFormat($item->amount_applied_vat, $PRNumber->currency, false)}} / {{$item->amount_applied_vat_rate}}
                @endif
            </td> --}}
            <td class="ac">
               @foreach ($getPRNumber as $PRNumber)
