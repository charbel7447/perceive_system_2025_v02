...
<section class="content" id="content">
 <div class="content-title">
      Payments Mode Report
     <?php $ReportId1 = App\PaymentOptionReport\PaymentOptionReport::latest()->take(1)->get(); ?>
     @foreach ($ReportId1 as $report)
         @if($report->from_date)
            @if($report->from_date != "1990-01-01")
             From: {{date('d-m-Y', strtotime($report->from_date))}} -
            @endif
         @endif
         @if($report->to_date)
            @if($report->to_date != "2030-01-01")
