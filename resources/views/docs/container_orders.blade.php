    ...
    <table class="summary">
                ...
                <td class="summary-info">
                    <table class="info">
                        <tbody>
                            ...
                            <tr>
                                <td>Total Weight:</td>
                                <td>{{moneyFormat($model->total_weight, $model->currency, false)}}</td>
                            </tr>
                            <tr>
                                <td>Total Volume:</td>
                                <td>{{moneyFormat($model->total_volume, $model->currency, false)}}</td>
                            </tr>
                        </tbody>
                    </table>
                </td>
        ...
        <table class="items">
            <thead>
                <tr>
                    <th >Item Code</th>
                    <th class="ar" >Weight Box</th>
                    <th class="ar" >Volume Box</th>
            ...
            <tbody>
                @foreach($model->items as $item)
                    <tr>
                        ...
                        <td class="ac">{{$item->weight_box}}</td>
                        <td class="ac">{{$item->volume_box}}</td>
                        <td class="ar">
                            {{moneyFormat($item->unit_price, $model->currency, false)}}
                        </td>
                        <td class="ac">{{$item->quantity}} / {{$item->uom->unit}}
                        </td>
                        <td class="ar">
            ...
            <tfoot>
                <tr>
                    ...
                    <td colspan="1" class="center">
                      ...
                      <?php
                        $totalQty = App\PurchaseOrder\Item::where('purchase_order_id','=',$model->id)->sum('qty');
                      ?>
                      {{  $totalQty}}
                    </td>
                    <td colspan="1"></td>
                ...
                <tr>
                    @if($dispaly_vat == 1)
                    <td colspan="3" class="item-empty">{{$second_currency}} {{moneyFormat($model->totaltax * $model->exchangerate, $model->currency, false)}}</td>
                    @else
                    <td colspan="3"></td>
                    @endif
                    <td colspan="2">Vat:</td>
                ...
                <tr>
                    ...
                    <td>
                        <strong>{{moneyFormat($model->discount, $model->currency)}}</strong>
                    </td>
                </tr>
                <tr>
                    <td colspan="3" class="item-empty"></td>
                    ...
                    <td>
                        <strong>{{moneyFormat($model->shipping, $model->currency)}}</strong>
                    </td>
                </tr>
                <tr>
                    <td colspan="3" class="item-empty"></td>
