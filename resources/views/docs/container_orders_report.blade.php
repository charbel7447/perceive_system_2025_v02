...
<section class="content" id="content">
   <div class="content-title">
        Shippments Order Report
        <?php $ReportId1 = App\ContainerOrderReport\ContainerOrderReport::latest()->take(1)->get(); ?>
    ...
    <table class="items">
      ...
      <tbody>
            ...
            <?php $getPRNumber = App\ContainerOrder\ContainerOrder::where('id','=',$item->container_order_id)
                  ...
                  @foreach ($getPRNumber as $itemx)
                     <tr>
                      ...
                      <td class="ac">
                        {{$item->shippers->person}}
                        <small>{{$item->shippers->company}}</small>
                       </td>
                       <td class="ac">
                          <?php $getProductsID = App\ContainerOrder\Item::where('container_order_id','=',$item->container_order_id)->get(); ?>
                          @foreach ($getProductsID as $productname)
                               <?php $getProductsName = App\Product\Product::where('id','=',$productname->product_id)->get(); ?>
                                 <ul style="margin: 0px 0px 0px 10px; padding:0px;">
                                      ...
                                      <?php $getProductPrice = App\ContainerOrder\Item::where('container_order_id','=',$item->container_order_id)->where('product_id','=',$finalName->id)->get(); ?>
                                          @foreach ($getProductPrice as $unitPriceP)
                                              <strong>| QTY: </strong>{{$unitPriceP->quantity}}
                      ...
                      <td class="ac">
                          {{moneyFormat($item->container_orders->total, $item->container_orders->currency, true)}}
                      </td>
                      
                      <td class="ac">
                          {{$item->container_orders->date}}
                      </td>
                      <td class="ac">
                        {{$item->container_orders->created_by}}
                    </td>
            ...
            <tr class="ar">
                ...
                <td colspan="1"  class="ar">
                  <?php  $sum1 = 0; ?>
                    ...
                    <?php $total = $variants1->container_orders['total'];
                        $act_points1 = $total;
                        $sum1 += $act_points1; ?>
                    @endforeach
                   {{$sum1}}
