...
<section class="content" id="content">
 <div class="content-title">
    Debit Notes Report
     <?php $ReportId1 = App\DebitNoteReport\DebitNoteReport::latest()->take(1)->get(); ?>
 ...
 <table class="items">
   ...
   <tbody>
      @foreach ($reports as $item)
         <tr>
            <td class="ac">
               <?php $getPRNumber = App\DebitNote\DebitNote::where('id','=',$item->debit_note_id)
                  ->where('status_id','>=',1)
            ...
            <td class="ac">
             @foreach ($getPRNumber as $PRNumber)
              {{moneyFormat($item->debit_notes->amount_received, $PRNumber->currency, true)}}
             @endforeach
            </td>
            <td class="ac">
