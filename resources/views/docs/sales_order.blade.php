...
<section class="content" id="content">
    ...
    <table class="summary">
                    ...
                    <table class="info">
                        <tbody>
                            ...
                            <tr>
                                ...
                                @if($model->client->vat_status == 0) 
                                 <td>{{moneyFormat($model->total_amount, $model->currency)}}</td>
                                @endif
                                @if($model->client->vat_status == 1) 
                                    <td>{{$model->currency->code}}&nbsp;{{moneyFormat($model->total_amount, $model->currency, false)}}</td>
                                @endif
                            ...
                            <tr>
                                <td>Total {{$second_currency}} VAT:</td>
                                <td>
     ...
     <?php $display_vat = DB::table('settings')
                ->latest()->take(1)
                ->where('key','=','display_vat')
                ->value('value');
        ...
        <table class="items">
            <thead>
                <tr>
                    ...
                    <th class="ar" width="10%">quantity / UOM</th>
                ...
                @foreach($model->items as $item)
                    <tr>
                        ...
                        @foreach($item['taxes'] as $tax)
                             <tr class="item-tax">
                                <td class="ar" colspan="5"><span style="Text-align:left;float:left;">{{$tax->name}} @ {{$tax->rate}}%</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                <span style="float: right;text-align:right">{{$second_currency}} {{moneyFormat($item->quantity * $item->price * ($model->vatrate / 100 ) * $model->vatrate, $model->currency, false)}}</span>
                            </td></tr> 
                        @endforeach
            ...
            <tfoot>
                <tr>
                    ...
                    <td colspan="1" class="center">
                      ...
                      <?php
                        $totalquantity = App\SalesOrder\Item::where('order_id','=',$model->id)->sum('quantity');
                      ?>
                      {{  $totalquantity}}
                    </td>
                    <td colspan="1"></td>
                ...
                <tr>
                    ...
                    @if($model->client->vat_status == 2)  
                    <td>
                        <strong>{{moneyFormat($model->total_amount, $model->currency)}}</strong>
                    </td>
                    @endif
                    @if($model->client->vat_status == 1)  
                    <td>
    ...
    <table class="items" style="margin: 0% 0;">
        <thead>
            <tr>
               <?php 
                     $spell = Terbilang::make(round($model->total_amount,2));
                ?>
                <th width="15%">Only:</th>
