...
<head>
    <meta charset="UTF-8">
    <title>Stock Count #{{ $stockCount->id }}</title>

    {{-- Bootstrap 5 + Icons + Lato Font --}}
...
<div class="container py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="mb-0">
            Stock Count #{{ $stockCount->id }}
            <span class="text-muted fs-6">({{ $stockCount->count_date }})</span>
        </h2>
        <div class="d-flex gap-2">
            <a href="{{ route('stock_count.index') }}" class="btn btn-warning btn-sm">
                <i class="bi bi-arrow-left"></i> Go Back
            </a>
            <a href="{{ route('stock_count.variance', $stockCount->id) }}" class="btn btn-outline-primary btn-sm">
                <i class="bi bi-bar-chart"></i> Variance Report
            </a>
            @if(empty($stockCount->submitted_at))
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-body d-flex justify-content-between text-center">
            ...
            <div><strong>Remaining Products</strong><br><span id="remainingProducts">{{ $stockCount->products->count() }}</span></div>
        </div>
    </div>
    ...
    <div class="card shadow-sm">
        <div class="table-responsive">
            <table class="table table-striped table-bordered align-middle mb-0">
                ...
                <tbody>
                    @foreach($stockCount->lots as $p)
                        <tr id="row-{{ $p->id }}">
                            <td>{{ $p->code }}</td>
                            <td>{{ $p->category ? "({$p->category->id}) - {$p->category->name}" : $p->category_id }}</td>
                            <td>{{ $p->subCategory ? "({$p->subCategory->id}) - {$p->subCategory->name}" : $p->sub_category_id }}</td>
                            <td>{{ $p->subSubCategory ? "({$p->subSubCategory->id}) - {$p->subSubCategory->name}" : $p->sub_sub_category_id }}</td>
                            <td class="currentStock">{{ $p->current_stock }}</td>
                            <td class="invStock">{{ $p->inventoried_stock }}</td>
                            <td class="variance {{ $p->variance == 0 ? 'text-muted' : ($p->variance > 0 ? 'text-success' : 'text-danger') }}">
                                {{ $p->variance }}
                            </td>
                            <td class="scannedAt">{{ $p->scanned_at }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function confirmScan() {
        ...
        fetch(`{{ url('/stock-count/'.$stockCount->id) }}/confirm-ajax/${scannedProductId}`, {
            method: "POST",
            headers: {
    ...
    barcodeInput.addEventListener('keypress', function(e) {
        if (e.key === 'Enter') {
            ...
            fetch("{{ route('stock_count.scan.ajax', $stockCount->id) }}", {
                method: "POST",
...
document.getElementById('prodSubSub').innerText = data.sub_sub_category_id && data.sub_sub_category_name 
    ...
    submitCountBtn.addEventListener('click', function() {
        ...
        fetch(`{{ route('stock_count.submit', $stockCount->id) }}`, {
            method: "POST",
            headers: { "Content-Type": "application/json", "X-CSRF-TOKEN": "{{ csrf_token() }}" }
        }).then(res => res.json())
        .then(data => {
            if (data.success) {
                ...
                setTimeout(() => { window.location.href="{{ route('stock_count.index') }}"; }, 1500);
            } else {
