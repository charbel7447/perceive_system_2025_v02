<table class="items">
   ...
   <tbody>
    <?php $reports = \App\ClientPaymentReport\Item::where('report_id','=',$id)->get(); ?>
      @foreach ($reports as $item)
        <?php
            $number = App\ClientPayment\ClientPayment::where('id','=',$item->client_payment_id)->value('number'); 
            $client = App\Client::where('id','=',$item->client_id)->value('company'); 
            $date = App\ClientPayment\ClientPayment::where('number','=',$number)->value('payment_date'); 
