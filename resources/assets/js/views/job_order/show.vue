    ...
    <div v-if="show">
        <div class="panel">
            <div class="panel-heading">
                ...
                <div>
                    <router-link :to="`/job_order`" class="btn" title="Go Back">
                        <i class="fa fa-arrow-left"></i>
                    </router-link>
                    <router-link target="_blank" :to="`../docs/job_order/${model.id}/`" class="btn" title="Print Product Label">
                        <i class="fa fa-barcode"></i>
                    </router-link>
                    <router-link :to="`/job_order/${model.id}/edit`"
...
<script type="text/javascript">
    ...
    export default {
        ...
        beforeRouteEnter(to, from, next) {
            get(`/api/job_order/${to.params.id}`)
                .then(res => {
        ...
        methods: {
            deleteModel() {
                ...
                if(r != true) { return }
                this.$bar.start()
                byMethod('delete', `/api/job_order/${this.model.id}`)
                    .then(({data}) => {
                        if(data.deleted) {
                            this.$router.push('/job_order')
            ...
            setData(res) {
                ...
                this.$title.set(`job_order - ${this.model.code}`)
                this.$bar.finish()
                this.show = true
            }
        }
