    ...
    <div v-if="show">
        <div class="panel">
            <div class="panel-heading">
                ...
                <div>
                    <router-link :to="`/shippers`" class="btn" title="Go Back">
                        <i class="fa fa-arrow-left"></i>
                    </router-link>
                    <router-link :to="`/shippers/${model.id}/edit`"
                    ...
                    <div class="col col-4">
                        <div class="bg-grey">
                            ...
                            <div class="row">
                                ...
                                <div class="col col-10">
                                    <p v-if="model.vat_status === 0">Exclude VAT</p>
                                    <p v-if="model.vat_status === 1">Include VAT USD + LBP</p>
                                    <p v-if="model.vat_status === 2">Include VAT USD Only</p>
                                </div>
                            </div>
                            <hr>
...
<script type="text/javascript">
    ...
    export default {
        components: {MiniPanel, PurchaseOrder, ReceiveOrder},
        data() {
            return {
                show: false,
                purchaseOrderURL: `/api/mini/shippers/purchase_orders/${this.$route.params.id}`,
                purchaseOrderColumns: ['Number', 'Date', 'Reference', 'Amount', 'Status'],
                receiveURL: `/api/mini/shippers/receive_orders/${this.$route.params.id}`,
        ...
        methods: {
            deleteModel() {
                ...
                if(r != true) { return }
                this.$bar.start()
                byMethod('delete', `/api/shippers/${this.model.id}`)
                    .then(({data}) => {
                        if(data.deleted) {
                            this.$router.push('/shippers')
            ...
            setData(res) {
                ...
                this.$title.set(`shippers - ${this.model.text}`)
                this.$bar.finish()
                this.show = true
            }
        }
