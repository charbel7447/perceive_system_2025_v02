                        ...
                        <table class="document-table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="align-center">Purchase Qty</th>
                            ...
                            <tbody>
                                <tr v-for="item in model.items">
                                    ...
                                    <td class="width-1 align-center">
                                        {{item.purchase_qty}} {{item.uom_code}}
                                    </td>
                                    <td class="width-1 align-center">
                                        {{item.qty}} {{item.received_uom_code}}
                                    </td>
                                </tr>
                            </tbody>
                        </table>
...
<script type="text/javascript">
    ...
    export default {
        ...
        methods: {
            myFunctionLabel(e){		
                 window.open('/docs/receive_label_barcode/'+ e, "_blank",`width=600,height=400, left=400, top=250`);	
             },
            deleteModel() {
            ...
            markAs(status) {
                ...
                post(`/api/receive_orders/${this.model.id}/mark`, {status: status})
                    .then(({data}) => {
                        if(data.saved) {
                            window.open('/purchase_orders/'+this.model.purchase_order_id + '/receive_order/', "_self",`width=1024,height=800`);	
                            // get(`/purchase_orders/${this.model.purchase_order_id}/receive_order`)
