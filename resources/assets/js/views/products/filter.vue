    ...
    <div>
        <div class="col-md-12 row" style="margin: 0;padding: 0;">
            <div style="text-align: left;" class="col-md-6">
                ...
                <input type="checkbox" :value="1" v-model="showFilter" style="padding: 2px  0;;width: 10px;">
            </div>
            <div style="text-align: right;margin: 0;padding: 0;" class="col-md-6">
                ...
                <router-link style="border: none;max-height: 29px;background:darkslateblue" slot="create" to="products_catalogue/create"
                        class="btn btn-primary"  title="Report" target="_blank">
                     Print Catalogue Label &nbsp;&nbsp;&nbsp;<i class="fa fa-file-pdf-o"></i>
                </router-link>
            </div>
        </div>
...
<table class="table table-item table-items item items" v-if="showFilter == 1">
    ...
    <tfoot>
        ...
        <panel ref="panel" :heading="heading" :resource="resource">
            ...
            <tr slot-scope="props" @click="$router.push(`/products/${props.item.code}`)">
                    <td>{{ props.item.code }}</td>
                    <td> <img width="60" :src=" '/assets/uploads/media-uploader/'+props.item.thumbnail" onerror="this.src='/images/placeholder.png'" /></td>
                   ...
                   <td v-if="props.item.min_qty >= props.item.current_stock">
                            <span style="color:red;">Minimum Stock Reached </span> <strong style="color:red;">{{props.item.current_stock}}</strong>
                    </td>
                     <td v-if="props.item.min_qty < props.item.current_stock">
                           Available Stock: <strong style="color:red;">{{props.item.current_stock}}</strong>
                    </td>
                    <td>
...
import Vue from 'vue'
    ...
    export default {
        ...
        beforeRouteEnter(to, from, next) {
            get('/api/products/filter', to.query)
                .then(res => {
        ...
        methods: {
            ...
            search() {
                ...
                this.$router.push(`filter?product_name=${this.product_name}&vendor_id=${this.vendor_id}&category=${this.category}&price_min=${this.price_min}&price_max=${this.price_max}&quantity_greater=${this.quantity_greater}&status=${this.status}&product_dropdown1=${this.product_dropdown1}&product_dropdown2=${this.product_dropdown2}`);
            },
            onVendorUpdated(e) {
