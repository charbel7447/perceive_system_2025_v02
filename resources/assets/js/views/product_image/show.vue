    ...
    <div v-if="show">
        <div class="panel">
            <div class="panel-heading">
                ...
                <div>
                    <router-link :to="`/payment_request`" class="btn" title="Go Back">
                        <i class="fa fa-arrow-left"></i>
                    </router-link>
                    <router-link target="_blank" :to="`../docs/payment_request/${model.id}/`" class="btn" title="Print Product DataSheet">
                        <i class="fa fa-file-pdf-o"></i>
                    </router-link>
                    <router-link :to="`/payment_request/${model.id}/payment_request_image`"
                        class="btn" title="Attach Image">
                        <i class="fa fa-paperclip"></i>
                    </router-link>

                    <router-link :to="`/payment_request/${model.id}/edit`"
                        ...
                        <div class="panel" v-if="model.document">
                            ...
                            <div class="panel-body">
                                <a :href="`/uploads/${model.document}`" target="_blank">
                                    <img class="panel-image" :src="`/uploads/${model.document}`" style="width: 150px;">
                                </a>
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    export default {
        ...
        beforeRouteEnter(to, from, next) {
            get(`/api/payment_request/${to.params.id}`)
                .then(res => {
        ...
        methods: {
            deleteModel() {
                ...
                if(r != true) { return }
                this.$bar.start()
                byMethod('delete', `/api/payment_request/${this.model.id}`)
                    .then(({data}) => {
                        if(data.deleted) {
                            this.$router.push('/payment_request')
            ...
            setData(res) {
                ...
                this.$title.set(`payment_request - ${this.model.item_code}`)
                this.$bar.finish()
                this.show = true
                window.location.href = `/payment_request/${this.model.id}`;
            }
        }
    }
</script>
