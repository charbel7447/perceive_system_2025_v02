                        ...
                        <div class="row">
                            <div class="col col-4">
                                ...
                                <label style="color:red;display: contents;"><strong>Note:</strong></label>  Vat Included</br>
                                  <label style="color:red;display: contents;"><strong>Exchange Rate:</strong></label><strong v-if="base_currency == 1">1 {{first_currency}} = {{model.vatrate}} {{second_currency}} </strong>
                                </label>
                                ...
                                <br>
                                <label v-if="display_exchange_rate == 1" style="display: inherit;">
                                ...
                                <table class="document-summary">
                                    <tbody>
                                        ...
                                        <tr>
                                         ...
                                         <tr>
                                            <td>Total:</td>
                                            <td>{{model.total + model.shipping - model.discount  | formatMoney(model.currency, false)}}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                            ...
                            <tbody>
                                <template v-for="item in model.items">
                                    ...
                                    <tr class="item-tax-show" v-if="item.taxes.length && (model.client.vat_status == 1)" v-for="tax in item.taxes">
                                        <td  colspan="2" v-if="display_vat == 1">Vat +{{(Number(item.qty * Number(item.unit_price)* Number(model.vatrate)  ) * tax.rate / 100) | formatMoney(model.currency, false) }}{{second_currency}}</td>
                                        <td v-if="display_vat == 1" colspan="2"> {{tax.name}}</td>
                                        <td v-else colspan="4"> {{tax.name}}</td>
                                        <td>{{tax.rate}}%</td>
                                        <td>+{{(Number(item.qty * Number(item.unit_price)  ) * tax.rate / 100) | formatMoney(model.currency, true)}}</td>
                                    </tr>
                           ...
                           <tfoot>
                                ...
                                <tr v-if="(model.client.vat_status == 1)" v-for="(value, key) in selectedTaxes" class="item-selected-tax">
                                    ...
                                    <td class="align-right" colspan="2">
                                        {{value| formatMoney(model.currency, true)}}
                                    </td>
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <td class="align-right">
                                        <strong>
                                            {{GrantTotal  | formatMoney(model.currency)}}
                                        </strong>
                                    </td>
                                </tr>
                            </tfoot>
...
<script type="text/javascript">
    ...
    export default {
        computed: {
            ...
            GrantTotal(){
                 return  Number(this.model.total) + Number(this.model.shipping) - Number(this.model.discount) ;
            }
        },
