                ...
                <div>
                    ...
                    <router-link v-if="model.is_editable" :to="`/invoices/${model.id}/edit`"
                     ...
                     <dropdown title="More" ref="more">
                        ...
                        <li v-if="[2, 3].indexOf(model.status_id) >= 0">
                            <router-link :to="`/receipt_vouchers/create?client_id=${model.client_id}`">
                                Receipt Voucher
                            </router-link>
                        </li>
                        <li v-if="model.status_id != 10">
                            <router-link :to="`/invoices/${model.id}/customer_returns`">
                                Customer Returns
                            </router-link>
                        </li>
                            ...
                            <tbody>
                                <template v-for="item in model.items">
                                    <tr>
                                        ...
                                        <td class="width-1 align-center">{{item.quantity}}
                                            <small class="status status-void" v-if="item.invoice_qty_returned > 0">
                                            Returned: {{item.invoice_qty_returned}}
                                        </small>
                                    </td>
                                        <td class="width-1 align-right">
                                            <span v-if="item.discount_usd >= 0 && item.discount_per == 0">{{item.discount_usd}} $</span>
                                        ...
                                        <td class="width-2 align-right">
                                            <span v-if="item.discount_usd >= 0 && item.discount_per == 0">{{(item.quantity * item.price) - (item.discount_usd) | formatMoney(model.currency, false)}}</span>
                            ...
                            <tfoot>
                                ...
                                <tr v-if="(model.client.vat_status == 1)" v-for="(value, key) in selectedTaxes" class="item-selected-tax">
                                 ...
                                 <tr>
                                    <td colspan="3"></td>
                                    <td colspan="2">
                                        <strong>Discount <br> <small>(Fixed Amount)</small></strong>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="3"></td>
                                    <td colspan="2">
                                        <strong>Discount <small style="color:red">{{ model.discount_percentage }} %</small></strong>
                                    </td>
                                    <td colspan="2"  class="align-right">
                                        <strong>
                                            {{((model.sub_total) * (model.discount_percentage/100)) | formatMoney(model.currency)}}
                                        </strong>
                                    </td>
                                </tr>
