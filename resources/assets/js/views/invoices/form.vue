        ...
        <div class="panel">
            ...
            <div class="panel-body">
                ...
                <div class="row">
                    ...
                    <div class="col col-3" v-if="form.seller_id" >
                        <div class="form-group">
                            <label>Seller</label>
                            <typeahead :initial="form.seller"
                                :url="sellerURL"
                                @input="onSellertUpdate"
                            >
                            </typeahead>
                            <error-text :error="error.seller_id"></error-text>
                        </div>
                    </div>
                    <div class="col col-3" v-if="(form.seller_commission > 0)">
                        <div class="form-group">
                            <label>
                                Seller Commission
                            </label>
                            <input  type="text" class="form-control" v-model="form.seller_commission">
                            <error-text :error="error.seller_commission"></error-text>
                        </div>
                    </div>
                ...
                <table class="item-table">
                    <thead>
                        <tr>
                            ...
                            <th>Discount 
                                <br>
                                <span v-if="form.discount_per != 1">$ <input type="checkbox" value="1"  v-model="form.discount_usd"></span> 
                                <span v-if="form.discount_usd != 1">% <input type="checkbox" value="1" v-model="form.discount_per"></span>
                            </th>
                            <th>UOM</th>
                            <th>Total</th>
                        </tr>
                    ...
                    <tbody>
                        ...
                        <template v-for="(item, index) in form.items">
                             <tr>
                                <td style="vertical-align:middle;"> <a class="link_shortcut_2" v-if="item.product" @click="myFunction(item, index)"><i class="fa fa-link"></i></a></td>
                                ...
                                <td style="vertical-align:middle;"> <a class="link_shortcut_2" v-if="item.product" @click="myFunctionH(item, index)"><i class="fa fa-link"></i></a></td>
                                ...
                                <td :class="['width-1', errors(`items.${index}.price`)]" >
                                    ...
                                    <input @click.stop="changePrice" class="form-control" type="text" v-model="item.price"  @keyup.enter="addNewLine" />
                                    <error-text :error="error[`items.${index}.price`]"></error-text>
                                    <span class="invoice-cost-price">({{ item.cost_price }})></span>
                                     <input style="display:none" type="text" class="form-control" v-model="item.cost_price">
                                </td>
                                ...
                                <td v-if="form.discount_per != 1 && form.discount_usd != 1" :class="['width-1', errors(`items.${index}.discount_per`)]" >
                                ...
                                <td class="width-2">
                                    <span class="form-control align-right" v-if="form.discount_usd == 1">
                                        <!-- {{(item.quantity * (item.price) - item.discount_usd) | formatMoney(form.currency, true)}} -->
                                        {{(item.quantity * (item.price) - (item.quantity) * (item.discount_usd))}}
                                    </span>
                                    <span class="form-control align-right" v-if="form.discount_per == 1">
                                        {{(item.quantity * (item.price) - ((item.quantity * (item.price)) * item.discount_per/100))}}
                                    </span>
                                    <span class="form-control align-right" v-if="form.discount_per != 1 && form.discount_usd != 1">
                            ...
                            <tr class="item-tax" 
                            v-if="item.quantity > item.qty_on_hand && (invoices_available_qty == 1  && company_type == 0) || item.cost_price > item.price" >
                                ...
                                <td colspan="3" v-if="item.cost_price > item.price">
                                    <span><strong class="price_cost_lower"> <i class="fa fa-exclamation-triangle"></i>  Warning the item {{ item.product_name }} price is lower than cost price</strong></span>
                                </td>
                            </tr>
                            
                             <tr class="item-tax"  v-if="item.taxes.length && ( form.vat_status == 1 )" v-for="tax in item.taxes">
                                <td colspan="4" v-if="display_vat == 1">Vat +{{(Number(item.quantity * Number(item.price)* Number(form.vatrate)  ) * tax.rate / 100) }}{{second_currency}}</td>
                                <td colspan="4" v-else></td>
                                <td colspan="1">{{tax.name}}</td>
                                <td>{{tax.rate}}%</td>
                                <td>+{{(Number(item.quantity * Number(item.price) ) * tax.rate / 100)}}</td>
                            </tr>
                        </template>
                    </tbody>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td class="item-empty"></td><td class="item-empty"></td>
                        ...
                        <tr>
                            ...
                            <td class="item-footer" colspan="3"><b>Discount</b> (%)</td>
                            <td class="item-footer" colspan="2" style="">
                                    <input type="text" class="form-control" v-model="form.discount_percentage">
                             </td>
                        </tr>
                          <tr>
                            <td class="item-empty"></td>
                            ...
                            <td class="item-footer" colspan="4"><b>Shipping</b> (Fixed Amount)</td>
                            <td class="item-footer" colspan="2" style="">
                    ...
                    <div class="col col-6">
                        <div class="col col-12">
                                <div class="form-group">
                                    ...
                                    <span class="form-control">{{ form.payment_condition }}</span>
                                </div>
                            </div>
                        <div class="col col-12">
                                <div class="form-group">
                    ...
                    <div class="col col-6">
                     <div class="col col-12">
                        <div class="form-group">
                            ...
                            <span class="form-control">{{ form.delivery_condition }}</span>
...
<script type="text/javascript">
    ...
    export default {
        ...
        computed: {
            ...
            subTotal() {
                ...
                if(this.form.discount_usd == 1){
                    return this.form.items.reduce((carry, item) => {
                        return carry + (Number(item.price) * Number(item.quantity) - (Number(item.quantity) * Number(item.discount_usd)) )
                    }, 0)
                  }
                  ...
                  if(this.form.discount_per != 1 && this.form.discount_usd != 1){
                    return this.form.items.reduce((carry, item) => {
                        return carry + (Number(item.price) * Number(item.quantity) - ((Number(item.price) * Number(item.quantity))))
                    }, 0)
                  }
                ...
                // return this.subTotal + this.totalTax * 1507 - this.form.discount;
                 ...
                 //  return  this.subTotal + (Number(this.form.shipping))  - (Number(this.form.discount)) ;
                  ...
                  if(this.form.vat_status == 2){
                      return  this.subTotal + this.totalTax + (Number(this.form.shipping))  - (Number(this.form.discount)) - (Number(this.subTotal * (this.form.discount_percentage/100))) ;
                  }
                  // Usd and LBP
                  ...
                  //not vat
                       ...
                       return this.subTotal + (Number(this.form.shipping))  - (Number(this.form.discount)) - (Number(this.subTotal * (this.form.discount_percentage/100))) ;
                  }
                //   if(this.form.discount_usd == 1){
                //     return this.form.items.reduce((carry, item) => {
        ...
        methods: {
            changePrice() {
                const r = confirm("Are you sure change price!")
                if(r != true) { return }
                else{}
            },
            ...
            myFunctionH(item, index){		
                window.open('/products_history/'+this.form.client_id + '/products_history/'+ item.item_id, "_blank",`width=1024,height=800`);	
            },
            ...
            onClientUpdate(e) {
                ...
                Vue.set(this.form, 'paymentcondition', client.paymentcondition)
                Vue.set(this.form, 'paymentcondition_id', client.paymentcondition_id)
                Vue.set(this.form, 'payment_condition', client.paymentcondition_name)

                Vue.set(this.form, 'deliverycondition', client.deliverycondition)
                Vue.set(this.form, 'deliverycondition_id', client.deliverycondition_id)
                Vue.set(this.form, 'delivery_condition', client.deliverycondition_name)
            },
            ...
            onProductUpdated(item, index, e) {
                ...
                Vue.set(this.form.items[index], 'cost_price', product.cost_price)
            ...
            saveBarcode() {
            get(`/api/scan_barcode/get_info?search=${this.form.barcode_scan}`)
            .then(response => {
                ...
                if (
                    ...
                    (!firstItem.item_id || !firstItem.price)
                ) {
                ...
                this.form.items.push({
                    ...
                    unit_price: scanned_product.unit_price,
            ...
            onDeliveryConditionUpdated(e) {
                ...
                Vue.set(this.form, 'delivery_condition', deliverycondition.name)
            },
            onPaymentConditionUpdated(e) {
                ...
                Vue.set(this.form, 'payment_condition', paymentcondition.name)
            },
        }
    }
</script>
