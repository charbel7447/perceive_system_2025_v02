    ...
    <tfoot>
        <tr>
            ...
            <th style="text-align: left;">
                <a class="btn btn-primary" href="/sales_orders">Clear</a>
            </th>
        </tr>
    </tfoot>
</table>
        ...
        <panel ref="panel" :heading="heading" :resource="resource">
            <span slot="title">Sales Orders</span>
             <router-link  slot="create" to="sales_orders_report/create"
                        class="btn btn-primar"  title="Report" target="_blank">
                     Report By Criteria &nbsp;&nbsp;&nbsp;<i class="fa fa-file-pdf-o"></i>
           </router-link>
           <router-link  slot="create" to="sales_orders_report/show"
                        class="btn btn-primar"  title="Report" target="_blank">
            ...
            <tr slot-scope="props" @click="$router.push(`${resource}/${props.item.id}`)">
                  ...
                  <td class="width-2">{{ props.item.date }}</td>
                    <td class="width-2">{{ props.item.number }}</td>
                     ...
                     <td class="width-2">{{ props.item.total_amount | formatMoney(props.item.currency) }}</td>
                   <td class="width-2">{{ props.item.seller_id }}</td>
                    <td class="width-2">
...
<script type="text/javascript">
    ...
    export default {
        ...
        data() {
            return {
                ...
                heading: [
                    ...
                    {title: 'Seller', name: 'created_by', sort: true},
                    {title: 'Status', name: 'status_id', sort: true}
                ]
            }
        },
        beforeRouteEnter(to, from, next) {
            get('/api/sales_orders/filter', to.query)
                .then(res => {
        ...
        methods: {
            ...
            search() {
                get(`/api/sales_orders/filter?client_name=${this.client_name}&from_date=${this.from_date}&to_date=${this.to_date}&amount=${this.amount}&client_dropdown1=${this.client_dropdown1}&client_dropdown2=${this.client_dropdown2}&client_seller=${this.client_seller}`);
                this.$router.push(`filter?client_name=${this.client_name}&from_date=${this.from_date}&to_date=${this.to_date}&amount=${this.amount}&client_dropdown1=${this.client_dropdown1}&client_dropdown2=${this.client_dropdown2}&client_seller=${this.client_seller}`);
            },
            setData(res) {
