    ...
    <div v-if="show">
        <div class="panel">
            <div class="panel-heading">
                ...
                <div>
                    <router-link :to="`/categories`" class="btn" title="Go Back">
                        <i class="fa fa-arrow-left"></i>
                    </router-link>
                    <router-link :to="`/categories/${model.id}/edit`"  
                        class="btn" title="Edit">
                        <i class="fa fa-pencil"></i>
                    </router-link>
                      <router-link :to="`/docs/categories/${model.id}/`" target="_blank"
                        class="btn" title="Edit">
...
<script type="text/javascript">
    ...
    export default {
        ...
        data() {
            return {
                show: false,
                productURL: `/api/mini/categories/products/${this.$route.params.id}`,
                productColumns: ['Item Code', 'Material Name', 'Quantity','UOM', 'Warehouse','Vendor Name','Status','Date'],
                model: {
                    currency: {}
                },
                moment: moment,
        ...
        beforeRouteEnter(to, from, next) {
            get(`/api/categories/${to.params.id}`)
                .then(res => {
        ...
        methods: {
            deleteModel() {
                ...
                if(r != true) { return }
                this.$bar.start()
                byMethod('delete', `/api/categories/${this.model.id}`)
                    .then(({data}) => {
                        if(data.deleted) {
