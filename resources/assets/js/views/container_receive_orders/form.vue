                ...
                <div class="row">
                    <div class="col col-6">
                        <div class="form-group">
                            <label>Vendor</label>
                            <span class="form-control">{{form.shipper.text}}</span>
                        </div>
                    </div>
                    <div class="col col-3">
                        <div class="form-group">
                ...
                <table class="item-table">
                    <thead>
                        <tr>
                            <th>Item Code</th>
                            <th>Description</th>
                            <th>U.O.M <small>(Container)</small></th>
                            <th>Qty in Order</th>
                            <th>Qty Received</th>
                            <th>U.O.M <small>(Original)</small></th>
                        </tr>
                    </thead>
                    <tbody>
                        <template v-for="(item, index) in form.items">
                            <tr>
                                ...
                                <td :class="['width-1', errors(`items.${index}.uom2_id`)]">
                                        <typeahead :initial="item.uom2"
                                            @input="onuom2Update(item, index, $event)"
                                        >
                                        </typeahead>
                                        <input style="display:none" type="text" class="form-control" v-model="item.uom2_id">
                                        <error-text :error="error.uom2_id"></error-text>
                                        <!-- {{ item.uom }} -->
                                </td>
                            </tr>
                        </template>
...
<script type="text/javascript">
    ...
    function initializeUrl (to) {
        let urls = {
            'create': `/api/container_receive_orders/create`,
            'edit': `/api/container_receive_orders/${to.params.id}/edit`,
            'clone': `/api/container_receive_orders/${to.params.id}/edit?mode=clone`,
            'container_order': `/api/container_orders/${to.params.id}/edit?mode=receive_order`,
        }
    ...
    export default {
        ...
        data () {
            return {
                resource: '/container_receive_orders',
                store: '/api/container_receive_orders',
                method: 'POST',
                title: 'Create',
        ...
        methods: {
            ...
            onuom2Update(item, index, e) {
                const uom2 = e.target.value

                // uom
                Vue.set(this.form.items[index], 'uom2', uom2)
                Vue.set(this.form.items[index], 'uom2_id', uom2.id)
            },
            onVendorUpdate(e) {
