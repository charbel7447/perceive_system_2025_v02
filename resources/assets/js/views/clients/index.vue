...
<table class="table table-item table-items item items" v-if="showFilter == 1">
    ...
    <tfoot>
        ...
        <panel ref="panel" :heading="heading" :resource="resource">
            ...
            <router-link slot="create" to="clients_balance_report/create"
                        class="btn btn-secondary  btn-full"  title="Report" target="_blank">
                     Balances &nbsp;<i class="fa fa-file-pdf-o"></i>
           </router-link>
           <router-link slot="create" to="download_client_table"  class="btn btn-primar"  title="Report" target="_blank">
                Clients &nbsp;<i class="fa fa-file-excel-o"></i>
           </router-link>
           <router-link slot="create" to="/clients/create" class="btn btn-primary">
            ...
            <tr slot-scope="props" @click="$router.push(`${resource}/${props.item.id}`)">
                ...
                <td class="width-2">{{ props.item.account_code }}</td>
                <td class="width-3">{{ props.item.company }}</td>
                <td class="width-3">{{ props.item.email }}</td>
                <td class="width-3">{{ props.item.balance }}</td>
    ...
    export default {
        ...
        data() {
            return {
                ...
                heading: [
                    ...
                    {title: 'Code', name: 'account_code', sort: true},
        ...
        methods: {
            ...
            search() {
                ...
                this.$router.push(`clients/filter?client_name=${this.client_name}&city=${this.city}&state=${this.state}&zip=${this.zip}&balance=${this.balance}&client_dropdown1=${this.client_dropdown1}&client_dropdown2=${this.client_dropdown2}&client_seller=${this.client_seller}`);
            },
            setData(res) {
