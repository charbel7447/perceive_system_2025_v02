<template>
    <div v-if="show">
                        ...
                        <div class="stats">
                            <div class="stat">
                                <div class="stat-inner">
                                    ...
                                    <p class="stat-value">{{parseFloat(stats.account_receivable).toFixed(3)}}</p>
                                </div>
                            </div>
                            <div class="stat">
                                <div class="stat-inner">
                                    <h2 class="stat-title">Total Revenue</h2>
                                    <p class="stat-value">{{parseFloat(stats.total_revenue).toFixed(3)}}</p>
                                </div>
                            </div>
                            <div class="stat">
                                <div class="stat-inner stat-last">
                                    <h2 class="stat-title">Open Sales Orders</h2>
                                    <p class="stat-value">{{parseFloat(stats.open_sales_orders).toFixed(3)}}</p>
                                </div>
                            </div>
                        </div>
                        <div class="stats">
                            <div class="stat">
                                <div class="stat-inner">
                                    ...
                                    <p class="stat-value">{{parseFloat(stats.unpaid_invoices).toFixed(3)}}</p>
                                </div>
                            </div>
                            <div class="stat">
                                <div class="stat-inner">
                                    <h2 class="stat-title">Unused Credit</h2>
                                    <p class="stat-value">{{parseFloat(stats.unused_credit).toFixed(3)}}</p>
                                </div>
                            </div>
                            <div class="stat">
                                <div class="stat-inner">
                                    <h2 class="stat-title">undrawn Payments</h2>
                                    <p class="stat-value">{{parseFloat(stats.advance_payments).toFixed(3)}}</p>
                                </div>
                            </div>
                        </div>
                        <div class="stats">
                            <div class="stat">
                                <div class="stat-inner">
                                    <h2 class="stat-title">Last Payment Date</h2>
                                    <p class="stat-value">{{model.last_payment_date}}</p>
                                </div>
                            </div>
                            <div class="stat">
                                <div class="stat-inner">
                                    <h2 class="stat-title">Balance Status</h2>
                                    <p class="stat-value" v-if="model.balance_status < 90">{{model.balance_status}} Days</p>
                                    <p class="stat-value" style="background-color: orangered;padding: 10px;color:#fff" v-else>{{model.balance_status}} Days - Overdue</p>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <mini-panel :resource="creditnotesURL"
            :heading="creditnotesColumns">
            <div slot="title">
                Credit Notes 
            </div>
            <router-link :to="`/credit_notes/create?client_id=${model.id}`"
                class="btn btn-secondary btn-sm" slot="create">Create</router-link>
            <tr slot-scope="props" @click="$router.push(`/credit_notes/${props.item.id}`)">
                    <td>{{ props.item.number }}</td>
                    <td>{{ props.item.client.company }}</td>
                    <td>{{ props.item.payment_date }}</td>
                    <td> <span class="red">{{ props.item.payment_mode || '-' }} </span> {{props.item.payment_reference || '-' }}</td>
                    <td>{{ props.item.amount_received | formatMoney(props.item.currency)}}</td>
                    <td><credit-note :id="props.item.status_id" /></td>
                </tr>
        </mini-panel>
...
<script type="text/javascript">
    ...
    import CreditNote from '../../components/status/CreditNotes.vue'
    export default {
        components: {MiniPanel, Quotation, SalesOrder, Invoice, AdvancePayment, ClientPayment,CreditNote},
        data() {
            return {
                show: false,
                ...
                paymentColumns: ['Number','company', 'Date', 'Reference', 'Amount Received', 'Status'],
                creditnotesURL: `/api/mini/clients/credit_notes/${this.$route.params.id}`,
                creditnotesColumns: ['Number','company', 'Date', 'Reference', 'Amount Received', 'Status'],
