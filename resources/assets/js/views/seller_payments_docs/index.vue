<template>
    <div>
        <panel ref="panel" :heading="heading" :resource="resource">
            <span slot="title">Seller Payments Docs</span>
            <router-link  slot="create" to="seller_payments_docs_report/create"
                        class="btn btn-primar"  title="Report" target="_blank">
                     Report &nbsp;&nbsp;&nbsp;<i class="fa fa-file-pdf-o"></i>
           </router-link>
            <router-link slot="create" to="/seller_payments_docs/create" class="btn btn-primary">
                New
            </router-link>
            <tr slot-scope="props" @click="$router.push(`${resource}/${props.item.id}`)">
                ...
                <td class="width-4" :title="props.item.seller_id">
                    {{ props.item.seller_id | trim(40) }}
                </td>
                <td class="width-2">{{ props.item.total_amount_received | formatMoney(props.item.currency) }}</td>
...
<script type="text/javascript">
    ...
    export default {
        ...
        data() {
            return {
                ...
                heading: [
                    ...
                    {title: 'Seller', name: 'client', sort: false},
                    {title: 'Total Amount', name: 'amount_received', sort: true},
        ...
        beforeRouteEnter(to, from, next) {
            get('/api/seller_payments_docs', to.query)
                .then(res => {
