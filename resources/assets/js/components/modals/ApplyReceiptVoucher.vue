        ...
        <div class="modal-mask">
            <div class="modal-wrapper">
                <div class="modal-container2">
                    <div class="modal-heading">Apply Receipt Voucher to Invoices</div>
                            ...
                            <table class="item-table">
                                ...
                                <tbody>
                                    <tr v-for="(item, index) in form.items" :key="index">
                                        <td v-if="toNumber(item.invoice_id) > 0">
                                            <a :href="'/invoices/' + item.invoice_id">
                                            ...
                                            <input style="display:none;" type="text" v-model="item.invoice_id" />
                                            
                                        </td>
                                        <td v-else>
                                            <typeahead :initial="form.invoices"
                                                       :url="invoicesURL"
                                                       @input="onInvoiceUpdate(item, index, $event)">
                                            </typeahead>
                                        </td>
...
export default {
    ...
    data() {
        return {
            show: false,
            resource: `/receipt_vouchers/${this.$route.params.id}`,
            url: `/api/search/receipt_vouchers_invoices/${this.$route.params.id}`,
            store: `/api/receipt_vouchers_invoices/${this.$route.params.id}/apply`,
            method: 'POST',
            message: 'You have successfully applied payment to receipt_vouchers!',
            currencyURL: '/api/search/currencies',
            invoicesURL: '/api/search/invoices_confirmed',
            form: {
                currency: {},
                client: {},
                exchange_rate: 1,
    ...
    methods: {
        ...
        recomputeAllRunningBalances() {
            ...
            this.form.items.forEach((row) => {
                const invoiceId = row.invoice_id || `__no_invoice_${Math.random()}`
        ...
        onInvoiceUpdate(item, index, e) {
            const invoices = e.target.value

            Vue.set(this.form.items[index], 'invoice_id', invoices.id)
            Vue.set(this.form.items[index], 'number', invoices.number)
            Vue.set(this.form.items[index], 'date', invoices.date)
            Vue.set(this.form.items[index], 'total', this.toNumber(invoices.total))
            Vue.set(this.form.items[index], 'amount_paid', this.toNumber(invoices.amount_paid))
            Vue.set(this.form.items[index], 'status_id', invoices.status_id)
            Vue.set(this.form.items[index], 'invoices', invoices)

            // When linking an invoice, reset applied amounts for the row
            Vue.set(this.form.items[index], 'amount_applied', 0)
            this.computeAppliedUSD(this.form.items[index])
            this.recomputeAllRunningBalances()
        },
