...
public function updateSystemLive()
{
    ...
    $logs = [];
    $basePath = base_path(); // always Laravel root

    // ✅ Force all commands to run inside Laravel root
    $steps = [
        'git' => "cd {$basePath} && git fetch origin main 2>&1 && git reset --hard origin/main 2>&1",
        'composer' => "composer install --no-dev -o --working-dir={$basePath} 2>&1",
        'migrate' => "cd {$basePath} && php artisan migrate --force 2>&1",
        'npm' => "cd {$basePath} && npm install 2>&1 && npm run build 2>&1",
    ];

    try {
        foreach ($steps as $key => $cmd) {
            $logs[$key] = $this->runCommandLineByLine($cmd);

            // if composer/git/npm fails, stop immediately
            if ($key !== 'migrate' && preg_match('/fatal|error|ERR!/i', $logs[$key])) {
                return api([
                    'saved' => false,
                    'message' => ucfirst($key) . " step failed!",
                    'logs' => $logs
                ]);
            }
        }

        // ✅ Fetch latest version from GitHub VERSION.txt
        $latestVersion = $this->fetchLatestVersion();

        // ✅ Update .env with APP_VERSION
...
private function runCommandLineByLine($command)
{
    $output = '';
    $process = proc_open(
        $command,
    ...
    if (is_resource($process)) {
        ...
        while ($err = fgets($pipes[2])) { $output .= $err; }
        fclose($pipes[1]);
        fclose($pipes[2]);
        proc_close($process);
    }
